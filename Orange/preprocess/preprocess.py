...
class RemoveConstant(Preprocess):
    """
    Constructs a preprocessors which removes features with constant values
    from the data set.
    """
    def __call__(self, data):
        """
        Removes columns with constant values from the data set and returns
...
class PreprocessorList(object):
    """
    Stores a list of preprocessors and on call applies them to
    the data set.
