...
class AsyncCall(QObject):
    ...
    unhandledException = Signal(object)
    resultReady = Signal(object)
    ...
    def __init__(self, func=None, args=(), kwargs={}, thread=None,
        ...
        self._async_start.connect(self.execute, Qt.QueuedConnection)
    ...
    def execute(self):
        ...
        try:
            ...
            print("Exception in thread ", QThread.currentThread(),
                  " while calling ", self.func, file=sys.stderr)
            self.finished.emit(repr(ex))
            self.finished.emit(self, repr(ex))
            self.unhandledException.emit(sys.exc_info())
...
class _RunnableTask(QRunnable):
    ...
    def run(self):
        if isinstance(self._call, AsyncCall):
            self.eventLoop = QEventLoop()
            self.eventLoop.processEvents()
            self._call.finished.connect(lambda str: self.eventLoop.quit())
...
def createTask(call, args=(), kwargs={}, onResult=None, onStarted=None,
    ...
    if onResult is not None:
        async.resultReady.connect(onResult, Qt.QueuedConnection)
    if onStarted is not None:
        async.starting.connect(onStarted, Qt.QueuedConnection)
    if onFinished is not None:
        async.finished.connect(onFinished, Qt.QueuedConnection)
    if onError is not None:
        async.unhandledException.connect(onError, Qt.QueuedConnection)
