...
class TransformRunner:
    ...
    def run(
        ...
        if isinstance(data.Y, da.Array):
            data.Y = data.Y.persist()
        if isinstance(data.metas, da.Array):
            data.metas = data.metas.persist()
        return data


class OWDaskPersist(OWWidget, ConcurrentWidgetMixin):
    name = "Dask Persist"
    ...
    class Outputs:
        transformed_data = Output("Computed Data", DaskTable)
    ...
    def __init__(self):
        ...
        gui.label(
            box, self, """
The widget takes Data and computes it and stores it for efficient future access.
""".strip(), box=True)
...
if __name__ == "__main__":  # pragma: no cover
    ...
    WidgetPreview(OWDaskPersist).run(set_data=table)
