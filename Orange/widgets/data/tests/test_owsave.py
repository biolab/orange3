...
class TestOWSave(WidgetTest):
    ...
    def test_writer(self):
        compressions = [self.widget.controls.compression.itemText(i) for i in range(self.widget.controls.compression.count())]
        types = [self.widget.controls.filetype.itemText(i) for i in range(self.widget.controls.filetype.count())]
    ...
    def test_ordinary_save(self):
        ...
        for ext in FILE_TYPES.keys():
            ...
            with named_file("", suffix=FILE_TYPES[ext]) as filename:
