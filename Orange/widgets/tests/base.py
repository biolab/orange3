...
class WidgetTest(WidgetTestBase):
    ...
    def test_has_keywords(self):
        ...
        if "keywords" not in widget_class.__dict__:
            self.fail(
                f"Widget {widget_class.__name__} must define a 'keywords'"
                f"class attribute.\n"
        ...
        source = None
        try:
            source = inspect.getsource(widget_class)
        except OSError:
            try:
        ...
        if source is not None \
                and re.search(r"^\s+keywords\s*=\s*\[", source, re.MULTILINE):
            self.fail(
                "'keywords' class attribute must be a comma-separated string, "
                "not a list." + file_line)
