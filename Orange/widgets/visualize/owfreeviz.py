...
class OWFreeVizGraph(OWScatterPlotBase):
    ...
    def _show_indicator_event(self, ev):
    ...
    def _update_point_items(self):
        self.__remove_point_items()
        self.__add_point_items()

    def _update_circle_item(self):
        self.__remove_circle_item()
        self.__add_circle_item()

    def _update_indicator_item(self, point_i):
        self.__remove_indicator_item()
        self.__add_indicator_item(point_i)

    def __remove_circle_item(self):
        if self._circle_item is not None:
            self.plot_widget.removeItem(self._circle_item)
            self._circle_item = None

    def __add_circle_item(self):
        if not len(self._points):
            return
        r = self.radius / 100 + 1e-5
    ...
    def __remove_point_items(self):
        for axitem in self._point_items:
            self.plot_widget.removeItem(axitem)
        self._point_items = []

    def __add_point_items(self):
        r = self.radius / 100 + 1e-5
    ...
    def __remove_indicator_item(self):
        if self._indicator_item is not None:
            self.plot_widget.removeItem(self._indicator_item)
            self._indicator_item = None

    def __add_indicator_item(self, point_i):
