...
class OWParallelGraph(OWPlot, orngScaleData):
    ...
    def updateData(self, attributes, midLabels = None, updateAxisScale = 1):
        ...
        for name in list(self.selectionConditions.keys()):        # keep only conditions that are related to the currently visualized attributes
        ...
        for i in range(dataSize):
            ...
            if 0 in [data[index] >= self.selectionConditions[name][0] and data[index] <= self.selectionConditions[name][1] for (name, index) in list(conditions.items())]:
    ...
    def showDistributionValues(self, validData, indices):
        ...
        for attr in indices:
            ...
            if self.dataDomain[attr] == self.dataDomain.classVar:
                ...
                maxVal = max(maxVal, max([max(val or [1]) for val in list(self.domainContingency[attr].values())] or [1]))
