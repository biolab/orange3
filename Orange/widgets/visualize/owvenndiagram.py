    ...
    def commit(self):
        ...
        for i, (key, input) in enumerate(self.data.items()):
            ...
            if not self.output_duplicates:
                id_column = np.array([[instance_key(row)] for _, row in subset.iterrows()],
                                        dtype=object)
                source_names = np.array([[names[i]]] * len(subset),
                                           dtype=object)
        ...
        if annotated_data_subsets:
            ...
            indices = np.hstack(annotated_data_masks)
...
def table_concat(tables):
    ...
    for table in tables:
        new_table.extend(Table.from_table(domain, table))
    ...
    for i, inst_id in enumerate(ids):
        ...
        for var in domain.variables + domain.metas:
            if var in idvarlist or var in in_expanded:
                continue
            if np.isnan(instance[var]):
                for ind in indices:
                    if not np.isnan(table.loc[table.index[ind], var]):
