...
class EllipticEnvelopeTest(unittest.TestCase):
    def setUp(self):
        ...
        self.X_in = 0.3 * np.random.randn(self.n_true_in, 3)
        self.X_out = np.random.uniform(low=-4, high=4,
                                       size=(self.n_true_out, 3))
        ...
        self.cls = self.learner(self.X_all)

    def test_EllipticEnvelope(self):
        y_pred = self.cls(self.X_all)
    ...
    def test_mahalanobis(self):
        n = len(self.X_all)
        y_pred = self.cls(self.X_all)
        y_mahal = self.learner.mahalanobis(self.X_all.X)
