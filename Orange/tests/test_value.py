...
class TestValue(unittest.TestCase):
    ...
    def test_compare_continuous(self):
        auto = Table('auto-mpg')
        acc1 = auto.iloc[0]['acceleration']  # 12
        acc2 = auto.iloc[1]['acceleration']  # 11.5
    ...
    def test_compare_discrete(self):
        data = Table(Domain([DiscreteVariable(name="G",values=["M", "F"])]),
                     np.array([[0],[1]]))
        self.assertTrue(data.iloc[0]['G'] < data.iloc[1]['G'])
        self.assertTrue(data.iloc[0]['G'] >= data.iloc[0]['G'])
        self.assertTrue(data.iloc[0]['G'] < 1)
        self.assertTrue(data.iloc[0]['G'] < "F")
        self.assertFalse(data.iloc[1]['G'] < "F")

    def test_compare_string(self):
        zoo = Table('zoo')
        zoo1 = zoo.iloc[0]['name']  # aardvark
        zoo2 = zoo.iloc[1]['name']  # antelope
