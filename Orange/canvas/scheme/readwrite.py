...
def gen_exec(links, index, inited_nodes, nodes, widget_names):
    """ Creates Python code to pass data between generated widget objects """
    print(inited_nodes)
    snippet = []
    node = nodes[index]

    def init_node(node):
        inited_nodes.append(node)
        print(inited_nodes)
        name = widget_names[nodes.index(node)]
        snippet.append("_" + name + " = " + name + "()")
        snippet.append(name + ".get_output()")

    def process_link(link):
        if link.sink_node not in inited_nodes:
            init_node(link.sink_node)
        source_index = nodes.index(link.source_node)
        sink_index = nodes.index(link.sink_node)
        source_name = widget_names[source_index]
        sink_name = widget_names[sink_index]
        line = sink_name + ".set_input(\"" + link.sink_channel.name
        line += "\", " + source_name + ".outputs[\"" + link.source_channel.name
        line += "]\")"
        snippet.append(line)

    if node not in inited_nodes:
        init_node(node)
    for link in links:
        if link.source_node == node:
            process_link(link)
    return "\n".join(snippet) + "\n"
...
def scheme_to_python(scheme, out_file):
    ...
    All widgets in a scheme are converted to a linear list of tasks that
    ...
    sorted_nodes = scheme_toposort(scheme)
    widget_names = []
    # Convert the nodes in the list to their corresponding widgets
    sorted_widgets = list(map(scheme.widget_manager.widget_for_node, sorted_nodes))
    for i, widget in enumerate(sorted_widgets):
        widget_names.append(widget.name.replace(" ", "_") + str(i))
    import_set = set()
    # List of snippets to be executed to perform script actions
    exec_code = ""
    inited_nodes = []
    pre_text = "#Script generated by Orange3\n\n"
    script = ""

    # Process each widget in order
    for i, widget in enumerate(sorted_widgets):
        if not(hasattr(widget, "init_code_gen")):
            print("No code generator for widget " + widget.name)
            continue
        gen = widget.init_code_gen()
        gen.set_name(widget_names[i])
        code_segments = gen.generate()
        # Imports and other code to go at the top of the script
        if code_segments[0]:
            # only unique imports will be added since it's a set
            import_set |= code_segments[0]
        # Widget core code and processing functions
        if code_segments[1]:
            script += str(code_segments[1]) + "\n"

        exec_code += gen_exec(scheme.links, i, inited_nodes, sorted_nodes, widget_names)
    ...
    out_file.write(pre_text + script + exec_code)
