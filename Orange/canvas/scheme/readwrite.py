...
from ...widgets.utils.codegen import strip_declaration
...
def get_parent_nodes(links, viable_nodes, adjs):
    parents = []
...
def scheme_toposort(scheme):
    ...
    def iter_satisfied(satisfied_nodes, sorted_scheme=[]):
        ...
        while len(satisfied_nodes) > 0:
            ...
            child_links = get_links_from(parent_node, adjs)
            for child_link in child_links:
                satisfied = True
                req_links = get_links_to(child_link[1], adjs)
    ...
    parent_nodes = get_parent_nodes(scheme.links, viable_nodes, adjs)
    ordered_scheme = iter_satisfied(parent_nodes)
    return ordered_scheme, adjs
...
def scheme_to_python(scheme, out_file):
    ...
    for i, widget in enumerate(sorted_widgets):
        ...
        gen.set_name(widget_names[i].lower().replace(" ", "_"))
        matched_links = []
        # Add all preambles to preamble set
        preambles |= set(gen.preambles)
        # Find all links with current node as sink
        for link in scheme.links:
            if link.sink_node == sorted_nodes[i]:
